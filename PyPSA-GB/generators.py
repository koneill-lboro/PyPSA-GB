...
def write_generators(time_step, year, networkmodel=True):
    ...
    df_pp_LOPF = df_pp.drop(columns=["x", "y", "Geolocation"])
    ...
    df_res_offshore = df_res_offshore.rename(
        columns={
    ...
    if year > 2020:
        ...
        df_res_offshore = pd.concat(
            [df_res_offshore, df_pipeline, df_future_FBOW, df_future_FOW],
            ignore_index=True,
        )

    df_res_offshore_UC = df_res_offshore.drop(columns=["x", "y"])
    df_res_offshore_UC["bus"] = "bus"

    df_res_offshore_LOPF = df_res_offshore.drop(columns=["x", "y"])
    ...
    df_res_onshore = df_res.loc[
        df_res["Technology Type"] == "Wind Onshore"
    ].reset_index(drop=True)
    ...
    df_res_onshore = df_res_onshore.rename(
        columns={
            "Installed Capacity (MWelec)": "p_nom",
            "Technology Type": "carrier",
            "Site Name": "name",
    ...
    df_res_onshore_LOPF = df_res_onshore.drop(columns=["x", "y"])
    ...
    df_res_PV = df_res.loc[
        df_res["Technology Type"] == "Solar Photovoltaics"
    ].reset_index(drop=True)
    ...
    df_res_PV_UC = df_res_PV.drop(columns=["x", "y"])
    df_res_PV_UC["bus"] = "bus"

    df_res_PV_LOPF = df_res_PV.drop(columns=["x", "y"])
    ...
    # UC bus is same
    df_hydro_UC = df_hydro.drop(columns=["x", "y"])
    df_hydro_UC["bus"] = "bus"

    df_hydro_LOPF = df_hydro.drop(columns=["x", "y"])
    ...
    df_NDC = df_NDC.rename(columns={"lon": "x", "lat": "y"})

    # UC bus is same
    df_NDC_UC = df_NDC.drop(columns=["x", "y"])
    df_NDC_UC["bus"] = "bus"

    df_NDC_LOPF = df_NDC.drop(columns=["x", "y"])
    ...
    df_bio = df_bio.rename(columns={"lon": "x", "lat": "y"})

    # UC bus is same
    df_bio_UC = df_bio.drop(columns=["x", "y"])
    df_bio_UC["bus"] = "bus"

    df_bio_LOPF = df_bio.drop(columns=["x", "y"])
    ...
    df_LOPF["name"] = df_LOPF["name"].astype(str).str.replace("\xa0", "")
    ...
    df_UC["name"] = df_UC["name"].astype(str).str.replace("\xa0", "")
...
def generator_additional_data(df, time_step):
    ...
    df.loc[:, "ramp_limit_up"] = (
        np.select(conditions, ramp_limit_up_) * (time_step * 60) / 100
    )
...
def write_generators_p_max_pu(
    start, end, freq, year, FES=None, year_baseline=None, scenario=None
):
    ...
    tech = "Wind Offshore"
    if year <= 2020:
        df_offshore = renewables.historical_RES_timeseries(year, tech, future=False)[
            "norm"
        ]
    elif year > 2020:
        df_offshore = renewables.future_offshore_timeseries(
            year, year_baseline, scenario, FES
        )["norm"]
    ...
    tech = "Wind Onshore"
    if year > 2020:
        df_onshore = renewables.historical_RES_timeseries(
            year_baseline, tech, future=True
        )["norm"]
        ...
        df_onshore = renewables.historical_RES_timeseries(year, tech, future=False)[
            "norm"
        ]

    df_onshore = df_onshore.loc[start:end]

    # KO amendment - adding next line of code to fix onshore wind error which doesn't \
    # correctly resample hourly atlite data to correct time_step, in particul half-hourly
    # 1. Ensure the index is a datetime object (this is often handled correctly,\
    # but good practice)
    df_onshore.index = pd.to_datetime(df_onshore.index)
    # 2. Explicitly set the frequency to 'h' (hourly) before conditional \
    # resampling occurs
        ...
        if year_baseline % 4 == 0:
            ...
            if year % 4 != 0:
                ...
                df_onshore = df_onshore[
                    ~((df_onshore.index.month == 2) & (df_onshore.index.day == 29))
                ]
    ...
    if year > 2020:
        df_PV = renewables.historical_RES_timeseries(year_baseline, tech, future=True)[
            "norm"
        ]
        # edit the years on the start and end to match the baseline year
    ...
    if year > 2020:
        ...
        if year % 4 != 0:
            # remove 29th Feb
            df_hydro = df_hydro[
                ~((df_hydro.index.month == 2) & (df_hydro.index.day == 29))
            ]
    ...
    df.columns = df.columns.astype(str).str.replace("\xa0", "")
    ...
    for c in carriers:
        for b in buses:
            ...
            for gen in list_of_sites:
                ...
                try:
                    gen_p_bus.append(
                        df_gen_p.loc[:, gen] * df_carrier_bus.loc[gen, "p_nom"]
                    )
                except KeyError:
                    pass
            try:
