...
def read_future_profile_data():
    ...
    df_eload = (
        df["ELOAD"]
        .drop([0, 1, 2, 3, 4, 5])[["eLOAD Model (2050).2"]]
    ...
    df_new_eload = pd.DataFrame(
        ...
        index=pd.date_range(
            start="2050-12-31 23:30:00", end="2050-12-31 23:30:00", freq="0.5h"
        ),
    )
...
def write_loads_p_set(
    ...
    if dataset == "historical":
        ...
        if year % 4 == 0:
            ...
            df_leap_day = df_hd[
                pd.to_datetime("2040-02-28 00:00:00") : pd.to_datetime(
                    "2040-02-28 23:30:00"
                )
            ]
            df_leap_day.index = pd.to_datetime(
                {
                    "year": df_leap_day.index.year,
